...
procedure Part2 is
    ...
    function Antinode_Count (A, B   : in Coordinate;
        ...
        procedure Place_Nodes (X_Start, Y_Start : in Positive;
                               X_Step,  Y_Step  : in Integer) is
            X : Integer := X_Start;
            Y : Integer := Y_Start;
        begin
            while X in Puzzle'Range(2) and then Y in Puzzle'Range(1) loop
                if not Has_Node (Y, X) then
                    Result := @ + 1;
                    Has_Node (Y, X) := True;
                end if;
                X := @ + X_Step;
    ...
    begin
        Place_Nodes (A.X, A.Y, -X_Off, -Y_Off);
        Place_Nodes (B.X, B.Y,  X_Off,  Y_Off);
        return Result;
    end Antinode_Count;

    Part2_Count : Natural := 0;
        ...
        begin
            for A in First_Index (Ants) .. Last_Index (Ants) - 1 loop
                for B in A + 1 .. Last_Index (Ants) loop
                    Part2_Count := @ + Antinode_Count (Ants (A), Ants (B), Key (C));
                end loop;
            end loop;
        end;
    end loop;

    Put_Line ("Part 2 count: " & Part2_Count'Image);
end Part2;
